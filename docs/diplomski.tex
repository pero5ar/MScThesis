...
\renewcommand{\lstlistingname}{Isječak kôda}
\renewcommand{\lstlistlistingname}{Popis isječaka kôda}
...
Programski jezici u kojima je sustav implementiran su JavaScript za poslužiteljsku stranu \engl{sever-side} i TypeScript za aplikacijsku, odnosno klijentsku stranu \engl{client-side}.
...
Sintaksa jezika je slična ostalim jezicima takozvane C obitelji jezika, ali za razliku od njih riječ je o dinamički tipiziranom \engl{dynamicly typed} interpretabilnom jeziku \citep{mdn_js}.
...
Na početku je u pripravnosti \engl{pending} i ovisno o uspješnosti asinkrone operacije odlazi u stanje \emph{ispunjen} \engl{fulfilled} ili \emph{odbijen} \engl{rejected} (slika \ref{fig:promise_lifecycle}).
...
Time se postiže neblokirajuće čekanje na izvršavanje asinkrone operacije, ekvivalentno tome da se pozvalo \emph{then}\footnote{
    Transpilator u praksi ne prevodi \emph{await} i kôd koji slijedi u \emph{then} već prevodi cijelu \emph{async} funkciju u funkciju generator \engl{generator function} gdje samo \emph{await} zamjeni s \emph{yield}.
...
TypeScript je programski jezik razvijen od strane tvrtke Microsoft, koji je nadogradnja \engl{superset} jezika JavaScript.
...
Budući da je TypeScript samo nadogradnja JavaScript-a, TypeScript datoteke mogu bez ikakvog problema uvoziti \engl{import} JavaScript datoteke i obratno.
Prevoditelj će u takvoj situaciji samo tretirati sve tipove unutar JavaScript datoteke kao tip \emph{any} koji može označavati bilo koji tip, osim ako prevoditelj nema pristup deklaracijskim datotekama \engl{declaration files} za funkcije i varijable koje se izvoze \engl{export} i koriste unutar JavaScript datoteke.
...
Drugi broj je \emph{Minor} i on ukazuje na to da su se dogodile promjene u funkcionalnosti i/ili sučelju paketa, ali je verzija kompatibilna sa svim nižim verzijama koji imaju isti \emph{Major} broj \engl{backwards compatible}, odnosno u slučaju da se koristi novija \emph{Minor} verzija od predviđene funkcionirat će jednako.
...
Poslužitelj se pokreće i izvodi u izvršnom okruženju \engl{runtime} \emph{Node.js}, a izgrađen je pomoću radnog okvira \engl{framework} Express.
...
Druga prednost je u konkurentnom model jezika JavaScript\footnote{poglavlje \ref{sec:async}} koji omogućava i potiče neblokirajuće, odnosno asinkrone ulazno-izlazne operacije \engl{asynchronous I/O}, što pomaže u izgradnji skalabilnih i brzih web-poslužitelja.
...
Svaka parna verzija ima podršku za održavanje \engl{long term support} koja počinje kada krene razvoj sljedeće neparne verzije.
...
Express (poznat još i kao \emph{Express.js}) je radni okvir \engl{framework} koji nadograđuje sučelje Node-a i module za gradnju web-poslužitelja s naprednim mogućnostima usmjeravanja \engl{routing} i opcionalnim korištenjem mehanizama predložaka \engl{template engines} za izgradnju web-stranica.
Napredne mogućnosti usmjeravanja olakšavaju razdvajanje kôda s obzirom na korijenske rute API-ja (korijenske resurse REST arhitekturnog stila), dodavanje međusloja sloja \engl{middleware} u cjevovod \engl{pipeline} specifičnih ruta, serviranje statičkih datoteka i konfiguriranje privremene memorije \engl{cache}.
...
Mongoose je implementiran tako da ovisi o i koristi službeni Mongo upravljački program \engl{driver} za Node, kojem se može pristupiti iz globalnog objekta Mongoose u slučaju da postoji potreba za nekim naprednim Mongo mogućnostima koje Mongoose sučelje ne nudi izravno\footnote{\url{https://docs.mongodb.com/ecosystem/drivers/node}}.
...
Izgled aplikacije definiran je primarno koristeći radni okvir \engl{framework} Bootstrap, a elementi radnog okvira korišteni su kao React komponente pomoću React Bootstrap implementacije.
...
Projekti u React-u često prate obrazac podijele komponenti na \emph{pametne} i \emph{glupe} odnosno komponente koje sadrže logiku (često nazvane \emph{container components} i prezentacijske komponente \engl{presentational components}.
...
\begin{enumerate}
    ...
    \item Faza promjena \engl{update}, u kojoj je instanca većinu vremena. U ovoj fazi događa se svako ponovno prikazivanje \engl{rerender} instance komponente, najčešće zbog promjene stanja ili ulaznih parametara (\emph{props}).
    \item Faza rastavljanja \engl{unmount} je završna faza komponente koja se događa kada se instanca uklanja iz Virtualnog DOM-a
\end{enumerate}
...
\begin{enumerate}[label=(\alph*)]
    ...
    Metode se redom pozivaju: neposredno nakon postavljanja komponente, neposredno prije poziva \emph{render} metode, u trenutku izvršavanja \engl{commit} povratne vrijednosti \emph{render} metode na DOM, neposredno nakon ažuriranja komponente, i neposredno prije rastavljanja komponente.
...
\begin{figure}[!htb] % Redoslijed poziva metoda životnog ciklusa React komponente
  \centering
...
\begin{figure}[!htb] % Struktura aplikacije stvorene naredbom „\emph{create-react-app moja-aplikacija -{}-typescript}“
    \centering
...
\newpage % Primjer korištenja React Router biblioteke
...
Jedini način za izmijeniti objekt je poslati \engl{dispatch} akciju.
Akcija \engl{action} je objekt koji opisuje promjenu stanja koja se želi postići i šalje se kao argument metode \emph{dispatch} \emph{store} singletona.
Kada objekt \emph{store} primi akciju poziva funkciju sažimanja \engl{reduce} koja je definira logiku koja se izvršava za traženu akciju.
...
Podatkovna i upravljačka logika čvorova, bridova i svih drugih elementa bibliteke sadržana je u objektima koji se zovu entiteti \engl{entity}, svaki od kojih nasljeđuje bazni razred \emph{BaseEntity}, dok je njihova vizualna reprezentacija, odnosno logika pogleda \engl{view logic} ostvarena pomoću React komponeti koji se nazivaju \emph{widgeti} i nasljeđuju baznu komponetu \emph{BaseWidget}.
...
\emph{DiagramModel} implementira logiku dodavanja i brisanja entiteta u dijagramu, dok \emph{DiagramEngine} sadrži tvornice \engl{factory} za stvaranje entiteta i implementira potpornu logiku koja je potrebna za crtanje elementa kao što je primjerice traženje najkraćeg puta između dva čvora.
...
Biblioteka uz bazne razrede spomenutih entiteta i \emph{widgeta} nudi i njihove \emph{zadane} \engl{default} verzije koje implementiraju neku osnovnu logiku koja bi bila potrebna za izradu osnovnog dijagrama, a također nudi i opcionalnu definiciju CSS klasa za te elemente kako bi se moglo što prije krenuti s razvojem \citep{storm_rd}.
U okviru ovog rada korištene su nadograđene verzije modela tih \emph{zadanih} elemenata.


\newpage
% Bootstrap i React Bootstrap
...
\begin{figure}[!htb] % Direktoriji i slojevi poslužiteljske aplikacije
    \centering
    ...
    \caption{
        ...
        Zelenom bojom označen je podatkovni sloj, plavom upravljački, sivom posredinčki \engl{middleware}, i žutom konfiguracije i ulazna datoteka.
    }
...
Korisnička shema nema jednostavan atribut \emph{password} kako je to prikazano u isječku kôda \ref{lst:schema}, već u sebi sadrži metodu postavljanja \engl{setter} koja osigurava da se u bazu ne sprema izravno korisnikova zaporka, već da se generira njen sažetak \engl{hash}.
...
Riječ je o API-ju koju implementiraju moderni internetski preglednici prvenstveno za potrebe čuvanja privremene memorije \engl{cache} u slučaju sporog interneta ili za ostvarivanje offline web-aplikacije\footnote{
...
Mehanizam dijagrama uvelike se oslanja na zadane \engl{default} elemente biblioteke STORM React Diagrams\footnote{poglavlje \ref{sec:storm}}.
...
Ostali zadani \engl{default} entiteti nisu zahtijevali proširivanje svojih razreda, ali zato razred \emph{DiagramModel} je.
...
\begin{figure}[!htb] % Isječak sučelja mehanizma crtanja dijagrama
  \razmaks
  \centering
...
\razmakp % Pojednostavljena verzija sučelja stanja aplikacije
...
\section{Primjer korištenja aplikacije}

Dolaskom na web-stranicu aplikacije korisniku će se prikazati forma za prijavu u sustav.
Nakon prijave bit će preusmjeren na tablicu sa zadacima koje može rješavati i može kliknuti na bilo koji od zadataka kako bi započeo ili nastavio rješavanje odabranog zadatka. Ako je rješavanje tek započelo, sučelje za rješavanje će izgledati kao na slici \ref{fig:screen1}.

\begin{figure}[!htb] % Sučelje za rješavanje zadatka na početku rješavanja
  \razmaks
  \centering
  \includegraphics[width=14.4cm]{images/screen1.png}
  \caption{Sučelje za rješavanje zadatka na početku rješavanja}
  \label{fig:screen1}
  \razmaks
\end{figure}

Na vrhu sučelja nalazi se naziv zadatka i gumbi koji omogućavaju predaju rješenja \engl{submit solution}, odnosno povratak na tablicu sa zadacima.
U oba će slučaja trenutno stanje dijagrama ostati sačuvano. 
U sredini sučelja je prostor za izgradnju dijagrama čiji lijevi rub sadrži gumbe koji omogućavaju stvaranje novih čvorova u dijagramu.
Ispod tog prostora nalazi se opis zadatka i ispod njega su na lijevoj strani postavke za trenutno odabrani čvor u dijagramu, a na desnoj stanje podataka, tj. tablica s podacima u tom čvoru dijagrama.
Na slici \ref{fig:screen1} niti jedan čvor nije odabran stoga nema postavki i podataka ispod opisa zadatka, ali na slici \ref{fig:screen2} je prikazan primjer s dodanim čvorom \emph{Start}.

\begin{figure}[!htb] % Sučelje za rješavanje zadatka s dodanim čvorom \emph{Start}
  \razmaks
  \centering
  \includegraphics[width=14.4cm]{images/screen2.png}
  \caption{Sučelje za rješavanje zadatka s dodanim čvorom \emph{Start}}
  \label{fig:screen2}
  \razmaks
\end{figure}

\newpage
\noindent Tekst zadatka prikazanog na slikama u ovom poglavlju glasi:

\begin{quotation}
    Prikaži redni broj, ime, prezime, i datum rođenja svih muških učenika koji su visoki barem 132 cm i imaju ispod 31 kg te svih ženskih učenica koje su visoke barem 131 cm i imaju ispod 30 kg
\end{quotation}

\noindent Na početku zadatka ponuđena je tablica učenika koja sadrži stupce: redni broj, ime, prezime prosjek, dan rođenja, mjesec rođenja, datum rođenja, spol, visina, i težina.

\razmakp

\noindent Slike u nastavku prikazuju odabrane korake izgradnje jednog mogućeg točnog rješenja zadatka:

\begin{enumerate}
    \item filtriranje svih učenika s vrijednosti visina većom ili jednakom 132 (slika \ref{fig:screen3})
    \item filtriranje svih muških učenika s vrijednosti visina većom ili jednakom 132 i vrijednosti težina manjom od 31 (slika \ref{fig:screen4})
    \item dodavanje čvora za uniju dosad izgrađene grane i one koja nedostaje (slika \ref{fig:screen5})
    \item filtriranje svih ženskih učenica s vrijednosti visina većom ili jednakom 131 i vrijednosti težina manjom od 30 (slika \ref{fig:screen6})
    \item spajanje novoizgrađene grane s čvorom Unija čime taj čvor sada sadrži sve retke koji su dani u uvjetu zadatka (slika \ref{fig:screen7})
    \item selekcija isključivo stupaca koji su zadani u opisu zadatka (slika \ref{fig:screen8})
    \item dodavanje čvora \emph{End} kako bi rješenje bilo spremno za predaju, tj. provjeru ispravnosti (slika \ref{fig:screen9})
\end{enumerate}

\razmaks

\noindent Konačno stanje rješenja prikazano je na slici \ref{fig:screen10}.

\razmakp

\begin{figure}[!htb] % Prvi odabrani korak izgradnje rješenja
  \razmaks
  \centering
  \includegraphics[width=14.4cm]{images/screen3.png}
  \caption{Prvi odabrani korak izgradnje rješenja}
  \label{fig:screen3}
  \razmaks
\end{figure}

\begin{figure}[!htb] % Drugi odabrani korak izgradnje rješenja
  \razmaks
  \centering
  \includegraphics[width=14.4cm]{images/screen4.png}
  \caption{Drugi odabrani korak izgradnje rješenja}
  \label{fig:screen4}
  \razmaks
\end{figure}

\begin{figure}[!htb] % Treći odabrani korak izgradnje rješenja
  \razmaks
  \centering
  \includegraphics[width=14.4cm]{images/screen5.png}
  \caption{Treći odabrani korak izgradnje rješenja}
  \label{fig:screen5}
  \razmaks
\end{figure}

\begin{figure}[!htb] % Četvrti odabrani korak izgradnje rješenja
  \razmaks
  \centering
  \includegraphics[width=14.4cm]{images/screen6.png}
  \caption{Četvrti odabrani korak izgradnje rješenja}
  \label{fig:screen6}
  \razmaks
\end{figure}

\begin{figure}[!htb] % Peti odabrani korak izgradnje rješenja
  \razmaks
  \centering
  \includegraphics[width=14.4cm]{images/screen7.png}
  \caption{Peti odabrani korak izgradnje rješenja}
  \label{fig:screen7}
  \razmaks
\end{figure}

\begin{figure}[!htb] % Šesti odabrani korak izgradnje rješenja
  \razmaks
  \centering
  \includegraphics[width=14.4cm]{images/screen8.png}
  \caption{Šesti odabrani korak izgradnje rješenja}
  \label{fig:screen8}
  \razmaks
\end{figure}

\begin{figure}[!htb] % Sedmi odabrani korak izgradnje rješenja
  \razmaks
  \centering
  \includegraphics[width=14.4cm]{images/screen9.png}
  \caption{Sedmi odabrani korak izgradnje rješenja}
  \label{fig:screen9}
  \razmaks
\end{figure}

\begin{figure}[!htb] % Konačno stanje rješenja
  \razmaks
  \centering
  \includegraphics[width=14.4cm]{images/screen10.png}
  \caption{Konačno stanje rješenja}
  \label{fig:screen10}
  \razmaks
\end{figure}
